...
# Query without time limits to get all recent runs (usually the last few)
wfs_url = (
...
# Extract beginPosition (origintime / phenomenonTime start)
origintimes = []
...
if not origintimes:
    # Fallback/alternative XPath if structure differs
...
if not origintimes:
    raise Exception("No model runs found – check FMI service status or XML structure")

latest_origintime = max(origintimes)
print(f"Latest model run (origintime): {latest_origintime}")

# --- Step 2: Download the NetCDF for the latest run (only analysis + short forecast to keep file small) ---
download_url = (
    "https://opendata.fmi.fi/download?"
    "producer=harmonie_scandinavia_surface&"
    "param=Temperature&"  # Capital T as in FMI examples
    "format=netcdf&"
    "bbox=19,56,30,61&"
    f"origintime={latest_origintime}&"
    f"starttime={latest_origintime}&"
    "timestep=60&"
    "timesteps=10&"  # Only first 9 hours + analysis (keeps file small; increase if needed)
    "projection=EPSG:4326&"
    "levels=2m"  # 2m temperature
)
...
print(f"Downloaded NetCDF ({os.path.getsize(nc_path)/1024/1024:.1f} MB)")

# --- Step 3: Load and convert temperature to °C ---
ds = xr.open_dataset(nc_path)
print("Available variables:", list(ds.data_vars))

# Temperature variable is usually 'Temperature' or 't2m' – auto-detect
if 'Temperature' in ds:
    temp_k = ds['Temperature']
elif 't2m' in ds:
    temp_k = ds['t2m']
else:
    raise Exception("Temperature variable not found in NetCDF")
...
if not os.path.exists(qml_path):
    raise FileNotFoundError("temperature_style.qml not found – please add your QGIS style file")

tree = ET.parse(qml_path)
root = tree.getroot()

items = []
# Common: colorrampshader items (discrete or gradient)
...
if not items:
    raise Exception("Could not parse color ramp from .qml – try exporting as text from QGIS instead")
...
# --- Step 5: Plot the analysis timestep (time=0) ---

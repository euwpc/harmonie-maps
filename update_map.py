...
import pandas as pdmatplotlib.use('Agg')# --- Helper to parse QML color ramp ---
def parse_qml_colormap(qml_file, vmin, vmax):
    ...
    return ListedColormap(colors), Normalize(vmin=vmin, vmax=vmax)# --- Step 1: Latest model run ---
...
tree = ET.fromstring(response.content)ns = {'gml': 'http://www.opengis.net/gml/3.2', 'omso': 'http://inspire.ec.europa.eu/schemas/omso/3.0'}
...
run_time_str = datetime.datetime.strptime(latest_origintime, "%Y-%m-%dT%H:%M:%SZ").strftime("%Y-%m-%d %H:%M UTC")# --- Step 2: Download with wide bbox ---
download_url = (
...
with open(nc_path, "wb") as f:
    f.write(response.content)# --- Step 3: Load data ---
ds = xr.open_dataset(nc_path)temp_c = ds['air_temperature_4'] - 273.15
...
precip_mm = ds['precipitation_amount_353']# --- Step 4: Load custom colormaps from QML files ---
temp_cmap, temp_norm = parse_qml_colormap("temperature_color_table_high.qml", vmin=-40, vmax=50)cape_cmap, cape_norm = parse_qml_colormap("cape_color_table.qml", vmin=0, vmax=5000)pressure_cmap, pressure_norm = parse_qml_colormap("pressure_color_table.qml", vmin=890, vmax=1050)windgust_cmap, windgust_norm = parse_qml_colormap("wind_gust_color_table.qml", vmin=0, vmax=50)precip_cmap, precip_norm = parse_qml_colormap("precip_color_table.qml", vmin=0, vmax=50)# Dewpoint uses temperature colormap
dewpoint_cmap = temp_cmap
dewpoint_norm = Normalize(vmin=-40, vmax=50)# --- Step 5: Helper ---
def get_analysis(var):
    if 'time' in var.dims:
        return var.isel(time=0)
    elif 'time_h' in var.dims:
        return var.isel(time_h=0)
    return var# --- Step 6: Views ---
views = {
...
for view_key, view_conf in views.items():
    ...
    suffix = view_conf['suffix']for var_key, conf in variables.items():
    # Analysis map
...
if os.path.exists("harmonie.nc"):
    os.remove("harmonie.nc")print("All maps + animations generated with custom colormaps")
